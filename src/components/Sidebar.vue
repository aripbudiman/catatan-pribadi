    ...
    <div class="p-4 border-b border-zinc-200">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-2 font-bold text-xl text-zinc-800">
          ...
          <span>Catatan Si URILLL</span>
        </div>
        <button v-if="!readonly" @click="emit('new')"
          class="p-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors" title="New Article">
          <Plus class="w-5 h-5" />
        </button>
      </div>
      <div class="relative">
    ...
    <div class="flex-1 overflow-y-auto p-2 space-y-6">
      <template v-for="category in categories" :key="category">
        <div v-if="filteredArticles.filter(a => a.category === category).length > 0" class="space-y-1">
          ...
          <button v-for="article in filteredArticles.filter(a => a.category === category)" :key="article.id"
            @click="emit('select', article.id)" :class="[
    ...
    <div class="p-4 border-t border-zinc-200 bg-zinc-100/50 space-y-3">
      <div v-if="user" class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-3 min-w-0">
          <div
            class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-xs shrink-0">
        ...
        <button @click="handleLogout"
          class="p-1.5 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Sign out">
